...
— and everything inherits the dialect the first three established: the tile, the plate, the band
...
gui/src/lib/book/             the book view's bands + three pure modules (items 27, 50)
...
gui/src/routes/               +page (library), book/[id], book/[id]/cards, cards, life
...
**The search is one box over both bands** (item 50), and this file previously
said the opposite twice — that `SearchMarks` had no `book_id`, which item 40
gave it, and that the input belonged beside the *Notes* heading. It does not.
`SearchMarks` returns **one ranked list over both indexes**, and the two things
it ranks are drawn by two different bands, so a box inside one of them is a
heading answering about the other. `MarkSearch.svelte` therefore belongs to the
column: a note hit opens the note pane, a passage hit scrolls to the passage and
marks it, and neither adds a fourth place to read something.

Three rules it carries. **The order is the engine's** — notes and passages
arrive interleaved and nothing above the seam re-sorts, re-groups or splits them
into sections, because two `source`-scoped calls spend one `limit` across two
lists whose lengths then vary with the query. **`limit` is required, and `0` is
an empty list** rather than *no limit* — `listBooks`, `listNotes` and this one
give a small number three different meanings, so a `?? 0` default here would be
a box that silently never finds anything. **The snippet is text, not markup**:
`>>`/`<<` around the terms, unescaped, possibly from a highlight's `ko_note` or
`annotation` rather than its `text`, un-mixed by `book/snippet.ts` — the only
parser of those markers anywhere, and `{@html}` appears nowhere near it.

A stale prohibition of exactly this kind survived a whole wave in this file
because every GUI thread is told to read it and believed it. When an item lands,
correct the text that said it could not.
