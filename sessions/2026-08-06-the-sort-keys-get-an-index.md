...
Item 34 of the non-GUI wave, alone in a worktree on `feat/engine-sort-keys`.
...
- **`sort_title` got a writer rather than a delete**, and the delete was a real
  ...
  `bindings.ts`, which item 33 owns this wave, and a half-delete (column gone,
...
- **`migration_versions_are_contiguous_from_one` fails on this branch**, and is
  expected to: `0015` is item 33's and has not merged. It is the one red test in
