...
Cargo **workspace**, six crates: `engine` (the library, package `readingbuddy`),
`tui` and `cli` (the two frontends, both linking the engine directly), `api`
(the versioned surface) and `daemon` (its transport), plus `corpus` (fixture
