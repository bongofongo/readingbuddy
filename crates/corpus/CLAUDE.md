...
- **`notes_fts` is written by the seed.** There are no triggers on that table —
  ...
  box exists to do.
- **`cover_path` is a whole path, so `gen-devdb` takes `--data-dir` as well as
  `--out`.** The two are different directories: `--out` is where the seed and the
  PNGs are *written*, `--data-dir` is where the library will be *built*, and the
  schema holds what `Storage::set_cover` would have stored
  (`images_dir.join(name)`) with nothing downstream joining a prefix back on.
  This was a relative path for a wave, on the argument that an absolute one names
  a single machine — but the seed is gitignored and regenerated by `make dev-db`
  on the machine that runs it, so the argument protected nothing while the cost
  was real: a relative path resolves only from a process whose cwd is the data
  dir, so `rb covers` measured none of the 202 files and a webview, which has no
  cwd at all, could resolve none of them either. Notes are the opposite and stay
  that way: `notes.file_path` is **vault-relative** by the engine's own
  convention, so the seed states a bare filename.
...
each one is for, so a screenshot reviewer knows what to look at.

**The set is declared in `edge-cases.json`, and this generator is checked against
it** (item 38). `gui/src/lib/api/fake.ts` models the same shapes for the
frontend's own layers and is checked against the *same file* from vitest, so
adding a hostile case to one fixture and not the other now fails on the side that
was not updated — it used to fail nowhere, and had already drifted four ways.
The declaration is **hand-edited and generated by nothing**: one emitted from
this generator would agree with it however wrong it was, which is this crate's
own oracle rule one level up. A case added here needs a line there and a line in
`fake.ts`, in any order; two of the three go red until the third lands.
