...
///
/// **Every arm is an `ORDER BY` over an index** since item 34 — see
...
pub enum BookSort {
    ...
    /// database's.
    ///
    /// Item 34 moved the parse to **write** time instead. `books.sort_author`
...
/// **`sort_title` left this table in item 34**, and that removal is the second
...
///
/// Item 34's whole mechanism, and the reason `sort_title` stopped being a column
...
/// out of step — and item 34 removed one, so the trap is not hypothetical.
...
impl Storage {
    ...
    pub async fn upsert_book(&self, book: &Book, source: Option<Source>) -> Result<i64> {
        ...
        // `sort_title` left the table in item 34 and this is where a
...
mod tests {
    ...
    fn order_by_is_a_total_order() {
        ...
        // `ORDER BY` at all and closed its own base read in Rust; since item 34
    ...
    ///
    /// It was two until item 34. `sort_title` sat here saying "ours, derived"
